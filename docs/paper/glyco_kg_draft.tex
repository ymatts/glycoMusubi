...
10-fold gap between cluster-level and exact-ID performance reveals a
quantitative determination boundary: protein sequence predicts structural class
...
Here we make three contributions. First, we construct \textbf{glycoMusubi}, a
heterogeneous knowledge graph integrating 78,263 nodes (10 types) and 2,508,960
edges (14 relation types) from six public databases through an automated,
reproducible ETL pipeline. Second, we establish a \textbf{hierarchical prediction
benchmark} comprising six tasks of increasing specificity, from binary N-linked
site classification to exact glycan identification over 2,357 candidates. Third,
we perform a \textbf{systematic investigation of cellular context} using
single-cell enzyme expression data, providing the first quantitative evidence
that current enzyme--glycan annotations are insufficient to bridge the
...
representation. Two variants are evaluated: site-level (using per-residue ESM-2
features at the glycosylation position) and protein-level (using mean-pooled
ESM-2 features over 3,345 candidates). Evaluation follows the filtered ranking
...
neighbourhood with high accuracy.
...
exact identification (Task~6) reveals a quantitative determination boundary.
...
\textsuperscript{\dag}Protein-level MRR exceeds site-level MRR because
mean-pooled ESM-2 features provide a smoother representation that acts as
implicit regularisation, offsetting the loss of site-specific positional
information; the larger candidate set (3,345 vs 2,357) includes O-linked
glycans, slightly altering the ranking landscape.}
...
the oracle achieves MRR = 0.026. The enzyme+popularity blend assigns optimal
...
Enzyme oracle & Within-cluster & 0.026 & 0.026 & 0.029 & Best cell type per sample \\
...
The hierarchical benchmark and negative-result analyses together support a
quantitative model of glycosylation determination with two distinct layers
...
it reflects an intrinsic property of glycosylation biology
...
framework: the 10-fold gap between cluster-level and exact-ID prediction is
consistent with the information-theoretic limit imposed by the number of
distinguishable enzymatic states in the Golgi.
...
(DOI:~10.5281/zenodo.XXXXXXX; full public release upon acceptance).
