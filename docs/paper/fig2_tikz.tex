...
model architectures.
(c)~Performance across all six benchmark tasks plus two retrieval variants
(site-level and protein-level) for exact glycan identification (7~rows;
6~distinct tasks). Dark bars represent sequence-determined tasks (Layer~1);
medium-gray bars represent environment-dependent tasks (Layer~2).}
\label{fig:twolayer}
\end{figure}
